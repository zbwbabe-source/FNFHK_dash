...
# 전처리된 CSV 파일 읽기
mlb_df = pd.read_csv('PL_MLB_2511_preprocessed.csv')
brand_df = pd.read_csv('PL_Brand_Summary_2511.csv')
...
def generate_ceo_insights():
    ...
    ceo_insights = {
        ...
        "executive_summary": {
            "title": "📊 핵심성과",
            "items": [
                f"• 11월 매출 성장: 실판매출 {net_cur:,.0f}K (YOY {net_yoy:.0f}%), 전년 동월 대비 {net_change:+,.0f}K",
...
# CEO Insights 저장
with open('public/dashboard/hongkong-ceo-insights-2511.json', 'w', encoding='utf-8') as f:
    json.dump(ceo_insights, f, ensure_ascii=False, indent=2)

# Dashboard Summary 저장
with open('public/dashboard/hongkong-dashboard-summary-2511.json', 'w', encoding='utf-8') as f:
    json.dump(dashboard_summary, f, ensure_ascii=False, indent=2)

print("완료: public/dashboard/hongkong-ceo-insights-2511.json")
print("완료: public/dashboard/hongkong-dashboard-summary-2511.json")
if mlb_summary:
    net_cur = mlb_summary['NET_CUR_MONTH']
    net_prev = mlb_summary['NET_PREV_MONTH']
    yoy = (net_cur / net_prev * 100) if net_prev > 0 else 0
    print(f"\n11월 실판매출: {net_cur:,.0f}K HKD (YOY {yoy:.1f}%)")
