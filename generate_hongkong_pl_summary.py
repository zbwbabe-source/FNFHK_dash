...
def get_mlb_sg_a(pl_data, period):
    """MLB 영업비 계산 (H99/M99 오피스의 판매관리비, BRD_CD='M'만)"""
    sg_a = 0.0
    for row in pl_data:
        if (row['PERIOD'] == period and 
            row['SHOP_CD'] in ['H99', 'M99'] and 
...
def get_dx_sg_a(pl_data, period):
    """DX(디스커버리) 영업비 계산 (H99/M99 오피스의 판매관리비, BRD_CD='X'만)"""
    sg_a = 0.0
    for row in pl_data:
        if (row['PERIOD'] == period and
            row['SHOP_CD'] in ['H99', 'M99'] and
...
def calculate_cumulative_store_data(pl_data, periods):
    """매장별 누적 데이터 계산 (급여, 임차료, 감가상각비 등 모든 직접비 항목)"""
    stores = {}
...
def main(target_period_short=None):
    ...
    print("\n손익 데이터 읽는 중...")
    # Period별 파일 찾기 (HKMC 폴더 구조 지원)
    import os
    import glob
    possible_pl_paths = [
        f'../Dashboard_Raw_Data/HKMC/{latest_period_short}/HKMC_PL_{latest_period_short}.csv',
        f'../Dashboard_Raw_Data/HKMC/{latest_period_short}/HKMC PL {latest_period_short}.csv',
    ...
    if not pl_csv_path:
        # glob으로 찾기
        glob_pattern = f'../Dashboard_Raw_Data/HKMC/{latest_period_short}/*PL*.csv'
